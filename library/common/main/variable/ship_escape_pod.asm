...
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION \ Enhanced: In the enhanced versions, the escape pod ship blueprint contains the information in the high nibble of byte #0 that scooping escape pods gives us slaves
...
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION OR _DISC_FLIGHT OR _ELITE_A_VERSION \ Advanced: The advanced versions of Elite have an extra edge count for the ship colour; escape pods are shown in cyan
...
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION \ Standard: Group A: In the cassette and Electron versions, the escape pod stores its faces with a scale factor of 8, while the other versions store them at a more accurate factor of 16
