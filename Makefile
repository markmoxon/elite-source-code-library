...
.PHONY:all
all: demo cassette disc 6502sp master electron elite-a nes c64 apple
ifneq ($(verify), no)
...
.PHONY:all-build
all-build: demo cassette disc 6502sp master electron elite-a nes c64 apple-build
ifneq ($(verify), no)
	@$(PYTHON) versions/demo/2-build-files/crc32.py versions/demo/4-reference-binaries/$(folder_demo) versions/demo/3-assembled-output
	@$(PYTHON) versions/cassette/2-build-files/crc32.py versions/cassette/4-reference-binaries/$(folder_cassette) versions/cassette/3-assembled-output
	@$(PYTHON) versions/disc/2-build-files/crc32.py versions/disc/4-reference-binaries/$(folder_disc) versions/disc/3-assembled-output
...
include versions/demo/Makefile
