...
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION \ Standard: Pythons in the cassette and Electron versions spawn up to 3 canisters on their demise, but this rises to 5 canisters in all other versions
...
ENDIF
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION \ Standard: Pythons in the cassette and Electron versions have a larger targetable area (120 x 120) than in all other versions (80 x 80)
...
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION OR _DISC_VERSION OR _ELITE_A_VERSION \ Advanced: The advanced versions of Elite have an extra edge count for the ship colour; Pythons are shown in cyan
 EQUB 85                \ Max. edge count          = (85 - 1) / 4 = 21
ELIF _6502SP_VERSION OR _MASTER_VERSION OR _C64_VERSION OR _APPLE_VERSION OR _NES_VERSION
 EQUB 89                \ Max. edge count          = (89 - 1) / 4 = 22
ENDIF
 EQUB 0                 \ Gun vertex               = 0
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION \ Standard: Pythons in the cassette and Electron versions have a slightly more glorious demise, with 10 expoding nodes compared to 9 nodes in all other versions
...
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION \ Standard: All Pythons in the cassette and Electron versions have a bounty of 200 Cr, whereas other versions have two types of Python: traders (with no bounty) and pirates (with a bounty of 200 Cr)
 EQUW 200               \ Bounty                   = 200
ELIF _6502SP_VERSION OR _DISC_VERSION OR _ELITE_A_DOCKED OR _MASTER_VERSION OR _C64_VERSION OR _APPLE_VERSION OR _NES_VERSION
 EQUW 0                 \ Bounty                   = 0
...
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION OR _DISC_DOCKED OR _ELITE_A_VERSION \ Standard: Group A: The Python has a number of different visibility settings for vertices, edges and faces between the cassette and disc ship hangar versions (which have slightly lower visibility values), and the other versions
...
IF _CASSETTE_VERSION OR _DEMO_VERSION OR _ELECTRON_VERSION OR _DISC_DOCKED OR _ELITE_A_VERSION \ Standard: See group A
