...
The repository also includes a flicker-free version that incorporates a fix for planets so they no longer flicker, though only when running on a BBC Master. The flicker-free code is in a separate branch called `bbc-master-flicker-free`.
...
The repository also includes a flicker-free version of the BBC Master version that incorporates a fix for planets so they no longer flicker. The flicker-free code is in a separate branch called `bbc-master-flicker-free`.
